...
double acos_calc_raw(int value_calc_size){

  double ans, raw_acos_calc_time;

  auto begin = std::chrono::system_clock::now();

  double incr = 1.0/(value_calc_size-1);
  double arg = -1.0;
...
int acos_cubic_spline_all_values(
  int look_up_size, int value_calc_size, 
    std::vector<std::vector<double>>& acos_look_up,
...
*/
  ...
  raw_acos_calc_time = acos_calc_raw(value_calc_size);  // raw calc of acos values

  acos_calc_lookup(look_up_size, acos_look_up);        // look-up table calc (y values)                  

  acos_cubic_spline_all_values(look_up_size, value_calc_size, acos_look_up, spline_acos_calc_time);
